...
El componente `<SisdaiSeriesTiempo/>` sirve para graficar datos temporales. En esta sección se explica su uso.
...
- `creaSeries`: Este método se ejecuta al montar el componente y cuando se detectan cambios en `datos`, `variables` o en las dimensiones del componente contenedor `<SisdaiGraficas>` y crea y actualiza el gráfico.
