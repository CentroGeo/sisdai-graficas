...
import { timeFormat, utcFormat } from './chunk-2DJ4GDBT.js'
import {
  second,
  timeDay,
  timeHour,
...
function transformer() {
  ...
  function scale(x) {
    return x == null || isNaN((x = +x))
      ...
      : (
          output ||
          (output = piecewise2(domain.map(transform), range2, interpolate))
        )(transform(clamp(x)))
  }
  scale.invert = function (y) {
    return clamp(
      untransform(
        (
          input ||
          (input = piecewise2(range2, domain.map(transform), number_default))
        )(y)
      )
    )
  }
...
function radial() {
  ...
  scale.copy = function () {
    return radial(squared.domain(), range2)
      .round(round)
      .clamp(squared.clamp())
      .unknown(unknown)
  }
...
function quantile2() {
  ...
  function scale(x) {
    return x == null || isNaN((x = +x))
      ? unknown
      : range2[bisect_default(thresholds, x)]
  }
...
function quantize() {
  ...
  function scale(x) {
    return x != null && x <= x
      ? range2[bisect_default(domain, x, 0, n)]
      : unknown
  }
  function rescale() {
    var i = -1
...
function threshold() {
  ...
  scale.domain = function (_) {
    return arguments.length
      ? ((domain = Array.from(_)),
        (n = Math.min(domain.length, range2.length - 1)),
        scale)
      : domain.slice()
  }
  scale.range = function (_) {
...
function sequentialQuantile() {
  ...
  function scale(x) {
    if (x != null && !isNaN((x = +x)))
      return interpolator(
        (bisect_default(domain, x, 1) - 1) / (domain.length - 1)
      )
  }
  scale.domain = function (_) {
    if (!arguments.length) return domain.slice()
